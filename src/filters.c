...
// Affiche chaque lettre du mot avec une couleur selon son état
void displayColorLetter(char* word, char* try) {
    for (int i = 0; i < MAX_WORD_LENGTH - 1; i++) {
        if (try[i] == 'v') {
            printf("\033[0;32m%c ", word[i]); // Vert
        } else if (try[i] == 'o') {
            printf("\033[0;33m%c ", word[i]); // Orange/jaune
        } else {
            printf("\033[0m%c ", word[i]);    // Normal
        }
    }
    printf("\033[0m\n");
}

// Récupère les mots depuis un fichier
...
// Vérifie si un mot est valide (existe dans la BDD)
...
// Vérifie les lettres et renvoie un tableau d'indices ('v', 'o', 'x')
void checkLetters(char* word, char* guess, char* try) {
    ...
    for (int i = 0; i < MAX_WORD_LENGTH - 1; i++) {
        if (try[i] != 'v' && wordCount[guess[i] - 'a'] > 0) {
            try[i] = 'o';
            wordCount[guess[i] - 'a']--;
        }
    }
...
// Applique un filtre : inclure certaines lettres
...
// Applique un filtre : lettre à une position précise
...
// Applique un filtre : mot contenant une sous-chaîne
void filterBySubstring(char words[TOTAL_WORDS][MAX_WORD_LENGTH], char* substring) {
    for (int i = 0; i < TOTAL_WORDS; i++) {
        if (words[i][0] == '\0') continue;

        if (!strstr(words[i], substring)) {
            words[i][0] = '\0';
        }
    }
}

// Affiche tous les mots restants après filtrage
...
void applyFilters(char originalWords[TOTAL_WORDS][MAX_WORD_LENGTH]) {
    ...
    // Copie initiale de la base de mots
    for (int i = 0; i < TOTAL_WORDS; i++) {
    ...
    while (1) {
        ...
        printf("\nOptions de filtre :\n");
        printf("1. Inclure des lettres\n");
        printf("2. Exclure des lettres\n");
        printf("3. Lettre à une position précise\n");
        printf("4. Contient une sous-chaîne\n");
